...
output "dns_records_to_create" {
  description = "Operator step when manage_dns=false: ACM validation CNAME(s) + the standard-origin CNAME to the ALB."
  value = merge(
    {
    ...
    { "standard-origin" = "CNAME ${var.demo_hostname} -> ${aws_lb.app.dns_name}" },
  )
}
