...
- [x] (2026-07-18 13:19Z) Completed the approved live portion of `td-2ad88f`: applied the saved seven-resource S3 bootstrap plan, initialized the lite root against its ignored mode-0600 backend configuration, observed native S3 lockfile creation/deletion, and produced a remote-backed 47-add/0-change/0-destroy lite plan. A fresh bootstrap plan is drift-free; direct AWS checks confirm AES256 encryption, versioning, Bucket Owner Enforced ownership, all public-access blocks, a nonpublic TLS/exact-principal policy, and 90-day noncurrent recovery. No lite runtime resource was applied. Receipt: `docs/receipts/aws-lite-state-backend.md`.
...
- A fresh security audit found one critical and six high advisories in the existing `@agent-native/core` → `better-auth@1.6.0` chain. The finding predates `infra/lite`, but public deployment would expose it, so `td-20a91a` now blocks U4 apply/cutover. Terraform-generated SSM values also make state sensitive; `td-2ad88f` owns the encrypted, access-controlled state backend required before apply.
- Terraform cannot produce an ordinary local plan after an active partial S3 backend is added but before its bucket exists. `infra/lite/local-plan.sh` resolves that bootstrap ordering problem by planning from a disposable copy that omits only `backend.tf`; the canonical root remains fail-closed on remote state.
- The supported Agent Native upgrader rewrites nonlocal dependency pins but does not refresh an already-`latest` lockfile resolution. A normal recursive package-manager update was required to move core from `0.105.0` to `0.109.2`. The workspace also carried a stale `better-auth: 1.6.0` override; removing it allowed core's supported `1.6.16` dependency to resolve and eliminated all high/critical audit findings.
...
- **Decision:** Land U4 infrastructure first in explicit no-apply bootstrap mode. **Rationale:** The add-only plan is reviewable without incurring cost; first cloud-init always stops the host, proof mode requires both a funded hosted key and immutable image, and no live apply may proceed until dependency and sensitive-state gates close.
- **Decision:** Protect lite state with one SSE-S3 bucket and Terraform's native S3 lockfile, using SSE-S3 instead of a customer-managed KMS key and no DynamoDB lock table. **Rationale:** This meets encryption, recovery, public-access, and concurrency requirements while adding only negligible S3 usage and no recurring KMS key or DynamoDB capacity cost. A separate local bootstrap root avoids a circular dependency and contains no application credentials. **Date/Author:** 2026-07-18 / Codex.
- **Decision:** Upgrade all Agent Native packages through the supported upgrader plus normal package-manager lock refresh, and remove the stale `better-auth` override instead of replacing it with a newer override. **Rationale:** Core `0.109.2` directly supports `better-auth 1.6.16`; letting that dependency resolve preserves upstream ownership and keeps future upgrades viable. **Date/Author:** 2026-07-18 / Codex.
...
- 2026-07-17: Completed the local U4 lite construction and add-only plan; added authenticated wake/lease controls, cost-fail-closed bootstrap, HTTPS/systemd/Compose runtime, plan-policy tests, provider lock, and explicit pre-apply security/state gates without creating AWS resources.
- 2026-07-18: Added the no-apply encrypted-state construction: exact-principal S3 bootstrap, version recovery, native lockfile backend, disposable local proof wrapper, and rotation/destruction runbook. Actual bucket creation and backend initialization remain behind the explicit cost-incurring apply gate.
- 2026-07-18: Removed the public-deploy dependency blocker by upgrading Agent Native through its supported workflow, removing the stale `better-auth 1.6.0` override, refreshing managed scaffold skills, and proving the workspace with audit, typecheck, 1,045 tests, and five builds.
- 2026-07-18: Completed the bounded live lite infrastructure proof: applied the 48-resource hibernating profile, published the immutable five-app image, proved authenticated wake/replay protection, all-five-app health, timer repair, and PostgreSQL persistence across stop/wake, then stopped the host. DNS/Cloudflare and hosted scoring remain gated.
