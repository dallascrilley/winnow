 ...
 * emits. Aggregates only: no lead ids, emails, or per-lead rows leave the
 * server. The generic panel-query pipeline stays auth-gated; this is the
...
const FUNNEL_SQL = `SELECT CASE event_name WHEN 'lead_submitted' THEN '1 submitted' WHEN 'lead_enriched' THEN '2 enriched' WHEN 'lead_scored' THEN '3 scored' WHEN 'lead_routed' THEN '4 routed' WHEN 'lead_booked' THEN '5 booked' ELSE event_name END AS stage, COUNT(*) AS n FROM analytics_events WHERE event_name IN ('lead_submitted','lead_enriched','lead_scored','lead_routed','lead_booked') GROUP BY event_name ORDER BY CASE event_name WHEN 'lead_submitted' THEN 1 WHEN 'lead_enriched' THEN 2 WHEN 'lead_scored' THEN 3 WHEN 'lead_routed' THEN 4 WHEN 'lead_booked' THEN 5 ELSE 9 END`;

const BY_DAY_SQL = `SELECT event_date AS date, COUNT(*) AS submissions FROM analytics_events WHERE event_name = 'lead_submitted' GROUP BY event_date ORDER BY date`;

const TIER_SQL = `SELECT COALESCE(properties::jsonb ->> 'tier', 'unknown') AS tier, COUNT(*) AS n FROM analytics_events WHERE event_name = 'lead_scored' GROUP BY tier ORDER BY n DESC`;

const SEGMENT_SQL = `SELECT COALESCE(properties::jsonb ->> 'segment', 'unknown') AS segment, COUNT(*) AS n FROM analytics_events WHERE event_name = 'lead_scored' GROUP BY segment ORDER BY n DESC`;

const TIME_TO_ROUTE_SQL = `WITH t0 AS (SELECT anonymous_id, MIN(timestamp::timestamptz) AS started FROM analytics_events WHERE event_name='lead_submitted' GROUP BY anonymous_id), t1 AS (SELECT anonymous_id, MIN(timestamp::timestamptz) AS routed FROM analytics_events WHERE event_name='lead_routed' GROUP BY anonymous_id) SELECT COUNT(*) AS routed_leads, COALESCE(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY EXTRACT(EPOCH FROM (t1.routed - t0.started))), 0)::int AS median_seconds FROM t0 JOIN t1 USING (anonymous_id)`;

const APPROVAL_LATENCY_SQL = `WITH p AS (SELECT anonymous_id, MIN(timestamp::timestamptz) AS parked FROM analytics_events WHERE event_name='lead_routing_proposed' AND properties::jsonb ->> 'band' = 'review' GROUP BY anonymous_id), d AS (SELECT anonymous_id, MIN(timestamp::timestamptz) AS decided FROM analytics_events WHERE event_name IN ('lead_approved','lead_rejected') GROUP BY anonymous_id) SELECT COUNT(*) AS decided, COALESCE(PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY EXTRACT(EPOCH FROM (d.decided - p.parked))), 0)::int AS median_seconds FROM p JOIN d USING (anonymous_id)`;

const PENDING_SQL = `SELECT COUNT(*) AS open FROM (SELECT anonymous_id FROM analytics_events WHERE event_name='lead_routing_proposed' AND properties::jsonb->>'band'='review' GROUP BY anonymous_id) p LEFT JOIN (SELECT anonymous_id FROM analytics_events WHERE event_name IN ('lead_approved','lead_rejected') GROUP BY anonymous_id) d USING (anonymous_id) WHERE d.anonymous_id IS NULL`;

const EVAL_SQL = `SELECT (properties::jsonb ->> 'accuracy')::float AS accuracy, properties::jsonb ->> 'model' AS model, properties::jsonb ->> 'caseCount' AS case_count, timestamp AS created_at FROM analytics_events WHERE event_name = 'eval_completed' ORDER BY timestamp DESC LIMIT 1`;

const RECENT_SQL = `SELECT timestamp AS time, properties::jsonb ->> 'tier' AS tier, properties::jsonb ->> 'segment' AS segment, properties::jsonb ->> 'fitScore' AS fit FROM analytics_events WHERE event_name = 'lead_scored' ORDER BY timestamp DESC LIMIT 12`;
